...
class Hinge(Piece):
    ...
    def draw_hinge(self, sketch, columns, rows):
        ...
        line1 = Line(Vector(2*D*(columns-0.5), D+self.extrude_height, 0), Vector(-D, D + self.extrude_height, 0), )

        sx_lower = Arc(Vector(0,-2*D*(rows-1),0), radius=M.hole_radius * 3, angle1=math.pi, angle2=math.pi*3/2)
        dx_lower = Arc(Vector(2*D*(columns-1), -2*D*(rows-1),0), radius=M.hole_radius * 3, angle1=math.pi/2*3, angle2=0)

        for i in range(columns):
            for j in range(rows):
                holes.append(Circle(Vector(2*D*i,-2*D*j,0), radius=M.hole_radius + 2 * M.tolerance))

        if columns > 1: 
            line2 = Line(Vector(0, -D*(rows+1), 0), Vector(2*D*(columns-1), -D*(rows+1), 0))
        else:
            line2 = sx_lower

        line3 = Line(Vector(-D, D + self.extrude_height, 0), Vector(-D, -D*(rows-1), 0))
        line4 = Line(Vector(2*D*(columns -0.5), -2*D*(rows-1), 0), Vector(2*D*(columns -0.5), D+self.extrude_height, 0))
        ...
        Constraints.distance((line1,LineSubParts.START_POINT), (line1, LineSubParts.END_POINT),2*D*(columns) )
        if columns > 1:
            Constraints.distance((line2,LineSubParts.START_POINT), (line2, LineSubParts.END_POINT),2*D*(columns-1) )
        ...
        Constraints.distance((line4,LineSubParts.START_POINT), (line4, LineSubParts.END_POINT),2*D*(rows-0.5) + M.medium_extrude_height )
        ...
        for i in range(rows):
            for j in range(columns):
                ...
                Constraints.distance_horizontal((hole,LineSubParts.CENTER_POINT), (Y, LineSubParts.START_POINT),-2*D*j)
                Constraints.distance_vertical((hole,LineSubParts.CENTER_POINT), (X, LineSubParts.START_POINT),2*D*i)
...
class TriangleHinge(Hinge):
    ...
    def draw_xz_sketch(self, sketch):
        ...
        d = e + Vector(2*D*(self.columns-0.5) / 2 - 2*D, 2*D*(self.n_rows_z-0.5))
        ...
        for j in range(self.n_rows_z):
            holes.append(Circle(center + j*Vector(0, D, 0), radius=M.hole_radius + 2 * M.tolerance))

        line1 = Line(a, b)
        line2 = Line(b, c)
        line3 = Line(c, d)
        line4 = Arc(center=Vector(2*D*(self.columns-0.5) / 2, 2*D*(self.n_rows_z-0.5), 0), radius=2*D, angle1=0, angle2=math.pi)
        line5 = Line(e, f)
        line6 = Line(f, a)
        ...
        Constraints.tangent((line3, LineSubParts.END_POINT), (line4, LineSubParts.START_POINT))
        # Constraints.tangent((line4, LineSubParts.END_POINT), (line5, LineSubParts.START_POINT))
        Constraints.coincident((line4, LineSubParts.END_POINT), (line5, LineSubParts.START_POINT))

        Constraints.points_horizontal((line4, LineSubParts.START_POINT), (line4, LineSubParts.END_POINT))

        Constraints.coincident((line1, LineSubParts.END_POINT), (line2, LineSubParts.START_POINT))
        Constraints.coincident((line2, LineSubParts.END_POINT), (line3, LineSubParts.START_POINT))
        # Constraints.coincident((line3, LineSubParts.END_POINT), (line4, LineSubParts.START_POINT))
        Constraints.coincident((line5, LineSubParts.END_POINT), (line6, LineSubParts.START_POINT))
        Constraints.coincident((line6, LineSubParts.END_POINT), (line1, LineSubParts.START_POINT))

        Constraints.coincident((fline1, LineSubParts.END_POINT), (fline2, LineSubParts.START_POINT))
        Constraints.coincident((fline2, LineSubParts.END_POINT), (fline3, LineSubParts.START_POINT))
        Constraints.coincident((fline3, LineSubParts.END_POINT), (fline4, LineSubParts.START_POINT))
        Constraints.coincident((fline4, LineSubParts.END_POINT), (fline1, LineSubParts.START_POINT))

        Constraints.coincident((gline1, LineSubParts.END_POINT), (gline2, LineSubParts.START_POINT))
        Constraints.coincident((gline2, LineSubParts.END_POINT), (gline3, LineSubParts.START_POINT))
        Constraints.coincident((gline3, LineSubParts.END_POINT), (gline4, LineSubParts.START_POINT))
        Constraints.coincident((gline4, LineSubParts.END_POINT), (gline1, LineSubParts.START_POINT))

        Constraints.coincident((line1, LineSubParts.START_POINT), (X, LineSubParts.START_POINT ))
        Constraints.distance((line1, LineSubParts.START_POINT), (line1, LineSubParts.END_POINT), 2*D*(self.columns))
        Constraints.distance((line2, LineSubParts.START_POINT), (line2, LineSubParts.END_POINT), self.edge_size)
        ...
        Constraints.coincident((holes[-1], LineSubParts.CENTER_POINT), (line4, LineSubParts.CENTER_POINT))

        for hole in holes[:-1]:
            Constraints.points_vertical((hole, LineSubParts.CENTER_POINT), (holes[-1], LineSubParts.CENTER_POINT))
        ...
        for i, hole in enumerate(holes):
            ...
            Constraints.distance_point_to_line((hole, LineSubParts.CENTER_POINT), X, (2*i+1)*D+self.extrude_height)
        ...
        Constraints.distance((fline1, LineSubParts.START_POINT), (X, LineSubParts.START_POINT), D)
        Constraints.distance((fline1, LineSubParts.START_POINT), (fline1, LineSubParts.END_POINT), D)
        ...
        Constraints.distance((gline1, LineSubParts.START_POINT), (line2, LineSubParts.START_POINT), D)
        Constraints.distance((gline1, LineSubParts.START_POINT), (gline1, LineSubParts.END_POINT), D)
        Constraints.parallel(gline3, line3)

        Constraints.points_horizontal((line5, LineSubParts.END_POINT), (fline4,LineSubParts.START_POINT ))
    ...
    def build(self, app):
        ...
        self.xz_sketch.MapMode = "FlatFace"
        self.xz_sketch.AttachmentSupport = [(self.xy_extruded,'Face1')]
        self.xz_extruded.Placement = App.Placement(App.Vector(-D,M.hole_radius*6 + 2*self.extrude_height),App.Rotation(App.Vector(0,0,1),180))
